...
class QuestionData(Dataset):
    ...
    def vectorize(ex : Iterable[str], word2ind : dict[str, int]):
        ...
        vec_text = [word2ind.get(w, word2ind['<unk>']) for w in ex]
...
class DanModel(nn.Module):
    ...
    def average(self, text_embeddings, text_len):
        ...
        # Compute the average embeddings by dividing by the number of words
        average = sum_embeddings / text_len.view(-1, 1).float()
    ...
    def forward(self, input_text, text_len, is_prob=False):
        ...
        word_embeddings = self.embeddings(input_text)
        result = word_embeddings.mean(dim=1)
        # logits = self.average(input_text, text_len)

        logits = self.network(result)
        if is_prob:
